...
function SetupDeliverPrompt()
    Citizen.CreateThread(
        function()
            local str = "Missão de Entrega"
            DeliverPrompt = PromptRegisterBegin()
            PromptSetControlAction(DeliverPrompt, 0x7F8D09B8)
    ...
    function()
        ...
        while true do
            ...
            if isInMarker and not hasAlreadyEnteredMarker then
                ...
                TriggerEvent("redemrp_Deliveryjob:hasEnteredMarker", currentZone)
            end

            if not isInMarker and hasAlreadyEnteredMarker then
                hasAlreadyEnteredMarker = false
                TriggerEvent("redemrp_Deliveryjob:hasExitedMarker", lastZone)
            end
        end
    end
)
...
AddEventHandler(
    "redemrp_Deliveryjob:hasEnteredMarker",
    function(zone)
        currentZone = zone
    end
)
...
AddEventHandler(
    "redemrp_Deliveryjob:hasExitedMarker",
    function(zone)
        if active == true then
            PromptSetEnabled(DeliverPrompt, false)
            PromptSetVisible(DeliverPrompt, false)
                ...
                if betweencoords < 2.2 then
                    ...
                    if pressed then
                        ...
                        if JobNum == 1 then
                            ...
                            Citizen.InvokeNative(0x9CB1A1623062F402, p1, "Entrega")
                            isDelivering = true
                            pressed = false
                            wag = Config.Cart1
                            wagon(wag)
                            TriggerEvent("VP:TOAST:New", "alert", "Procure pela bandeira branca perto de  " .. Config.StartWorking1)
                            ...
                            if IsPedInAnyVehicle(playerPed, false) then
                                ...
                                if model == -824257932 then
                                    if JobNum == 1 then --and IsPedInVehicle(playerPed, veh, true)
                                        -- TriggerEvent("VP:TOAST:New", "Você recebeu $" .. Config.Cash1 .. ", G" .. Config.Gold1 .. " G e " .. Config.Exp1 .. "XP",)
                                    ...
                                    if JobNum == 2 then
                                        ...
                                        -- TriggerEvent("VP:TOAST:New", "You got $" .. Config.Cash2 .. "," .. Config.Gold2 .. " G and " .. Config.Exp2 .. "XP",)
                                        isDelivering = false
                                        JobNum = nil
                                        RemoveBlip(p2)
                                        deletewagon()
                                    ...
                                    if JobNum == 3 then
                                        -- TriggerEvent("VP:TOAST:New", "You got $" .. Config.Cash3 .. "," .. Config.Gold3 .. " G and " .. Config.Exp3 .. "XP",)
                                    ...
                                    if JobNum == 4 then
                                        -- TriggerEvent("VP:TOAST:New", "You got $" .. Config.Cash4 .. "," .. Config.Gold4 .. " G and " .. Config.Exp4 .. "XP",)
                                    ...
                                    TriggerEvent("VP:TOAST:New", "alert", "Você está no veículo errado")
                                end
                            else
                                TriggerEvent("VP:TOAST:New", "alert", "Entre no veículo")
                            end
                        end
                    end
                end
    ...
    function()
        while true do
            ...
            if isDelivering and timer <= 0 then
                ...
                deletewagon()
                SetGpsMultiRouteRender(false)
                TriggerEvent("VP:TOAST:New", "alert", "Você não completou a entrega a tempo!")
            else
                timer = timer - 1000
            end
        end
