...
function cAPI.SpawnVehicle(model, cb, coords, isnetworked)
    ...
    local coords = coords ~= nil and coords or cAPI.GetCoords(GetPlayerPed(-1))
...
function cAPI.DeleteVehicle(vehicle)
...
function cAPI.TriggerCallback(name, cb, ...)
	LRPCore.ServerCallbacks[name] = cb
    TriggerServerEvent("LRPCore:Server:TriggerCallback", name, ...)
end

function cAPI.EnumerateEntities(initFunc, moveFunc, disposeFunc)
...
function cAPI.GetVehicles()
    local vehicles = {}
	for vehicle in cAPI.EnumerateEntities(FindFirstVehicle, FindNextVehicle, EndFindVehicle) do
		table.insert(vehicles, vehicle)
	end
	return vehicles
end

function cAPI.GetPeds(ignoreList)
    local ignoreList = ignoreList or {}
	local peds       = {}
	for ped in cAPI.EnumerateEntities(FindFirstPed, FindNextPed, EndFindPed) do
		local found = false
...
function cAPI.GetClosestVehicle(coords)
	...
	local vehicles        = cAPI.GetVehicles()
...
function cAPI.GetClosestPed(coords, ignoreList)
	local ignoreList      = ignoreList or {}
	local peds            = cAPI.GetPeds(ignoreList)
	local closestDistance = -1
    local closestPed      = -1
...
function cAPI.GetClosestPlayer(coords)
	if coords == nil then
        coords = GetEntityCoords(GetPlayerPed(-1))
	end
	
	local closestPlayers = cAPI.GetPlayersFromCoords(coords)
    local closestDistance = -1
...
function cAPI.GetPlayersFromCoords(coords, distance)
    local players = cAPI.GetPlayers()
    local closePlayers = {}
...
function cAPI.HasItem(source, cb, item)
	local retval = false
	cAPI.TriggerCallback('LRPCore:HasItem', function(result)
		if result then
			retval = true
		end
		return retval
...
function cAPI.GetVehicleProperties(vehicle)
...
function cAPI.SetVehicleProperties(vehicle, props)
