        ...
        for key, flag in pairs(flags) do
            ...
            for _ in pairs(loop) do
                ...
                if hit > 0 then
                    ...
                    if str then
                        TxtAtWorldCoord(endCoords.x, endCoords.y, endCoords.z, str, 2, 1)
                    end
                end
            end
        end
...
function DrawEntityInfo(entity)
    ...
    if carriedEntity then
        ...
        if carriedEntityHash then
            ...
            TxtAtWorldCoord(pCoords.x, pCoords.y, pCoords.z - 0.7, "Carrying: " .. carriedEntityHash .. " | " .. (GetHashName(carriedEntityHash) or ""), 0.475, 1)
        end
    end
    ...
    TxtAtWorldCoord(eCoords.x, eCoords.y, eCoords.z, str, 1.6, 3)
end
...
function DrawItemInfo(entity)
    ...
    local eRot = GetEntityRotation(entity)
    str = str .. '\nRot: x: ' .. (Floor(eRot.x * 100) / 100.0) .. ' y: ' .. (Floor(eRot.y * 100) / 100.0) .. ' z: ' .. (Floor(eRot.z * 100) / 100.0)
    TxtAtWorldCoord(eCoords.x, eCoords.y, eCoords.z, str, 0.6, 1)
end
